# MemoryGuard provider for Hermes Agent
...
1. Run MemoryGuard on `http://127.0.0.1:43847`.
2. Install/copy this plugin into the active Hermes external plugin location according to your Hermes version.
3. Put `MEMORYGUARD_URL=http://127.0.0.1:43847` in the Hermes profile `.env`.
4. Set the provider to `memoryguard` through Hermes' memory/plugin configuration.
5. Restart Hermes and check `hermes memory status`.

Optional: `MEMORYGUARD_NAMESPACE=personal-hermes`, `MEMORYGUARD_AUTO_CAPTURE=true`.
