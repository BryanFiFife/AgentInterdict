...
MemoryGuard ships an OpenClaw workspace skill and an agent-safe CLI. This is intentionally version-tolerant: OpenClaw's plugin/MCP surfaces have evolved, so the installer must inspect the installed OpenClaw release instead of writing speculative configuration keys.
...
After MemoryGuard has passed installation/tests, install the bundled skill:
...
- MemoryGuard recall does not bypass OpenClaw's normal action/tool approvals;
...
- persistent native memory paths not routed through MemoryGuard remain outside MemoryGuard's write veto and must be separately wrapped if full interception is required.
